  ...
  (parser
   ...
   (grammar
    ...
    (statement
     ...
     [(PAPER expr) (paper-expr $2 (numeric-expr 100) (numeric-expr 100))] ; paper expr for only one arg
     [(PAPER expr expr expr) (paper-expr $2 $3 $4)] ; paper exp for all 3 args
     [(PEN expr) (pen-expr $2)] ; pen expr
     [(REPEAT IDENTIFIER expr expr maybe-newlines block maybe-newlines) (repeat-expr $2 $3 $4 $6)] ; repeat exp (has structure of repeat line)
    ...
    (rvalues
     ...
     ;;; the second place you need to add variables, because they can also be r-values
     [(IDENTIFIER) (var-expr $1)]
     [(LEFTBRACKET expr expr RIGHTBRACKET) (get-paper-loc $2 $3)]
...
(check-expect (parse-str "Paper 100\n") (program (list (paper-expr (numeric-expr 100) (numeric-expr 100) (numeric-expr 100)))))
(check-expect (parse-str "Pen 100\n") (program (list (pen-expr (numeric-expr 100)))))
(check-expect (parse-str "Line 0 0 100 100\n")
              (program (list (line-expr (numeric-expr 0) (numeric-expr 0)
                                        (numeric-expr 100) (numeric-expr 100)))))
(check-expect (parse-str "Set X (50 * 25)\n")
              (program (list (assignment-expr (var-expr 'X)
                                              (mult-expr (numeric-expr 50) (numeric-expr 25))))))

(check-expect (parse-str "Repeat X 0 100\n{\nPrint X\n}\n")
              (program (list (repeat-expr 'X (numeric-expr 0) (numeric-expr 100)
                                          (list (print-expr (var-expr 'X)))))))
(check-expect  (parse-str "Set A X\n")
               (program (list (assignment-expr (var-expr 'A) (var-expr 'X)))))
