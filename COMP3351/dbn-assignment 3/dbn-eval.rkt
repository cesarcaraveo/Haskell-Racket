  ...
  (let ([prog (parse in)])
    (if (not (parser-error))
        ...
        (begin
          ...
          ; evaluate the statements, store the result and return it (for testing!)
          (let ([res (eval-statements (empty-env) (program-statements prog))])
...
(define (eval-file filename [env #f] [run-sim #t] [clear? #t])
  (let ([in (open-input-file filename)])
    (run-paper-sim)
    (let ([res (eval-dbn in env run-sim clear?)])
      (refresh)
      res)))
...
(define (eval-statement env statement [slow? #f])
  ...
  (match statement
    ...
    ;;; Add variable assignment, this requires using the environment
    ;;;       to see if it's there and creating it if it's not
    [(assignment-expr var val)
     (let ([variable (apply-env env var)] ; variable is either ref to var or #f
           [value (eval-expr env val)]) ; value is evaluated
       (if (false? variable) ; if variable is false (wasn't bound) 
           (extend-env env var value) ; create and bind variable
           (setref! (deref variable) value))) ; assign variable to value
     env] 
    ...
    [(command-fun sym params body)
     (let ([close (closure sym params body env)]) ; store closure 
       (extend-env env sym close) ; extend env with closure
       env)] ; return env
    ...
    [(apply-expr sym exprs)
     (let ([close (apply-env env sym)] ; checks to see if function is in environment (stores closure)
           [list (map (λ (exp) (eval-expr env exp)) exprs)]) ; list with all exprs evaluated
       (if (false? close)
           (error "function not found")
           (map (λ (el1 el2) (extend-env env el1 el2)) close list))
           env)]
    ))
...
; needed for check-expect
(require test-engine/racket-tests)

;(check-expect (eval-str "Paper 100\nPen 0\nLine 0 0 100 100\nValue [50 50]\n") 0)
;(check-expect (eval-str "Paper 0\nPen 100\nLine 0 0 100 100\nValue [50 50]\n") 100)
;(check-expect (eval-str "Set X 5\nValue X\n") 5)
;(check-expect (eval-str "Set X 5\nSet X 7\nValue X\n") 7)
; checks to see if commands definitions work--this should return a closure, the first example is a command with no arguments
(check-expect (eval-str "Command Foo { Set [20 20] 5 }\nValue Foo\n")
              (closure 'Foo '() (list (assignment-expr
                                       (get-paper-loc (numeric-expr 20) (numeric-expr 20))
                                       (numeric-expr 5))) (empty-env)))

; command definition with an argument 
#|(check-expect (eval-str "Command Foo X { Set [X X] 50 }\nValue Foo\n")
              (closure 'Foo '(X) (list (assignment-expr
                                        (get-paper-loc (var-expr 'X) (var-expr 'X))
                                        (numeric-expr 50))) (empty-env)))

; command definition with two arguments 
(check-expect (eval-str "Command Foo X Y { Set [X Y] 50 }\nValue Foo\n")
              (closure 'Foo '(X Y) (list (assignment-expr
                                        (get-paper-loc (var-expr 'X) (var-expr 'Y))
                                        (numeric-expr 50))) (empty-env)))

; command application with no arguments
(check-expect (eval-str "Command Foo { Set [20 20] 25 }\nFoo\nValue [20 20]")
              25)

; command application with one argument
(check-expect (eval-str "Command Foo X { Set [X X] 50 }\nFoo 25\nValue [25 25]\n")
              50)

; command application with two arguments
(check-expect (eval-str "Command Foo X Y { Set [X Y] 50 }\nFoo 25 35\nValue [25 35]\n")
              50)|#
