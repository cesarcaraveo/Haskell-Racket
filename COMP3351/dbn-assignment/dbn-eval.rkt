...
(define (eval-statement env statement [slow? #f])
  ...
  (match statement
    ...
    [(assignment-expr var val)
     (let ([variable (apply-env env exp)] ; check if variable is bound
           [value (eval-expr env val)]) ; evaluate value
       (if variable ; if variable is false (wasn't bound)
           (setref! variable value) ; assign variable to value
           (extend-env env variable value)) env)] ; create and bind variable
    ...
    [(command-fun sym params body)
     (let ([close (closure sym params body env)])
       (extend-env env sym close)
       env)]
     
    ; and we do the same thing for the numbers
    ;;; (Achievement) [(number-fun sym params body)
    ...
    [(apply-expr sym exprs)
     (let ([close (apply-env env sym)] ; checks to see if function is in environment (stores closure)
           [list (map (λ (exp) (eval-exp env exp)) exprs)]) ; list with all exprs evaluated
