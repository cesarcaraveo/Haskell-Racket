...
(define dbnlexer
  (lexer-src-pos
   ...
   [(:or "PAPER" "Paper")                           (token-PAPER)] ; added paper
   [(:or "PEN" "Pen")                               (token-PEN)] ; added pen
   [(:or "LINE" "Line")                             (token-LINE)] ; added line
   [(:or "SET" "Set")                               (token-SET)] ; added set
   ...
   [(:+ numeric) (token-NUMERICVALUE (string->number lexeme))] ; stores number representation of string
   ; identifiers
   ;;; Add identifiers, which should be a token-IDENTIFIER and contain the lexeme
   ; first has to be letter, rest can be letters or number
   ; :: concatanation (look at comments)
   [(::(:+ alphabetic) (:* any-string)) (token-IDENTIFIER (string->symbol lexeme))] ; identifier stars with letter, then any
...
; need to provide this for the check-expect to work
(require test-engine/racket-tests)

#|
; test cases for the lexer
(check-expect (lexstr "Paper") (list (token-PAPER)))
(check-expect (lexstr "PAPER") (list (token-PAPER)))
(check-expect (lexstr "Pen") (list (token-PEN)))
(check-expect (lexstr "PEN") (list (token-PEN)))
(check-expect (lexstr "Line") (list (token-LINE)))
(check-expect (lexstr "LINE") (list (token-LINE)))
(check-expect (lexstr "Set") (list (token-SET)))
(check-expect (lexstr "SET") (list (token-SET)))
; if this test fails, you're not converting your lexeme into a symbol, i.e., with string->symbol
(check-expect (lexstr "abc") (list (token-IDENTIFIER 'abc)))
(check-expect (lexstr "42") (list (token-NUMERICVALUE 42)))
(check-expect (lexstr "abc42") (list (token-IDENTIFIER 'abc42)))
(check-expect (lexstr "42abc") (list (token-NUMERICVALUE 42) (token-IDENTIFIER 'abc)))
(check-expect (lexstr "abc42abc") (list (token-IDENTIFIER 'abc42abc)))
(check-expect (lexstr "aBc42abC") (list (token-IDENTIFIER 'aBc42abC)))
